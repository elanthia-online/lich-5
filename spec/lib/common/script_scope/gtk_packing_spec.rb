...
RSpec.describe 'GTK compatibility shim (slice four: box packing)' do
  ...
  # GTK's expand: true, fill: false gives the child the room but keeps it
  # its natural size, centred in it. packing[:fill] was computed and never
  # read, so it rendered exactly like fill: true.
  describe 'expand without fill' do
    def packed(expand, fill, &setup)
      session.sync do
        row = gtk::Box.new(:horizontal)
        child = gtk::Button.new(label: 'Go')
        setup&.call(child)
        row.pack_start(child, expand, fill, 0)
        row.render_children
        child.send(:common_props)
      end
    end

    it 'centres the child in its column when it expands but does not fill' do
      expect(packed(true, false)[:align]).to eq('center')
    end

    it 'leaves a child that expands and fills to stretch' do
      expect(packed(true, true)).not_to have_key(:align)
    end

    it 'leaves a child that neither expands nor fills alone' do
      expect(packed(false, false)).not_to have_key(:align)
    end

    it 'keeps the child own halign over the packing' do
      expect(packed(true, false) { |child| child.halign = :end }[:align]).to eq('end')
    end

    it 'reaches the rendered tree' do
      window = session.sync do
        win = gtk::Window.new('t')
        row = gtk::Box.new(:horizontal)
        row.pack_start(gtk::Button.new(label: 'Go'), true, false, 0)
        win.add(row)
        win.show_all
        win
      end
      ...
      button = tree.each.find { |node| node.type == :button }

      expect(button.props[:align]).to eq('center')
    end
  end
  ...
  describe 'grid column weights' do
    ...
    # filler_for memoized by cell and filler_children returned every one ever
    # made, so the gone-child sweep in Container#materialize! never saw a
    # stale filler: a table that shrank kept adapter nodes for cells it no
    # longer had.
    describe 'a grid that shrinks' do
      def filler_nodes
        session.adapter.instance_variable_get(:@nodes).values.count do |node|
          node.type == :text && node.props[:content] == ' '
        end
      end

      it 'destroys the filler nodes for the cells it no longer has' do
        table = nil
        window = session.sync do
          win = gtk::Window.new('t')
        ...
        session.show_window(window)
        session.sync {}
        expect(filler_nodes).to eq(3)

        session.sync { table.n_columns = 2 }
        session.sync {}
        session.sync { nil }

        expect(filler_nodes).to eq(1)
        expect(table.filler_children.size).to eq(1)
      end

      it 'destroys a filler whose cell a child now covers' do
        table = nil
        window = session.sync do
          win = gtk::Window.new('t')
        ...
        session.sync { table.attach(gtk::Label.new('b'), 1, 2, 0, 1); table.show_all }
        session.sync {}
        session.sync { nil }

        expect(filler_nodes).to eq(0)
      end
    end

    # yoptions is fine to drop: rows are content-sized. A non-zero x or y
    # padding changes the layout visibly, and was dropped without a word.
    describe 'the attach padding the contract cannot carry' do
      around do |example|
        previous = gtk.unsupported_report
      ...
      def attach_entries
        gtk.unsupported_report.values.flat_map { |entries| entries.select { |api, _| api == 'Gtk::Table#attach' }.values }
      end

      it 'reports a non-zero padding once in the unsupported ledger' do
        session.sync do
          table = gtk::Table.new(2, 2)
          table.attach(gtk::Label.new('a'), 0, 1, 0, 1, nil, nil, 4, 0)
          table.attach(gtk::Label.new('b'), 1, 2, 0, 1, nil, nil, 0, 3)
        end

        expect(attach_entries.size).to eq(1)
        expect(attach_entries.first).to include(count: 2, note: 'xpadding/ypadding are ignored')
      end

      it 'says nothing about a zero-padding attach' do
        session.sync do
          table = gtk::Table.new(2, 2)
          table.attach(gtk::Label.new('a'), 0, 1, 0, 1, nil, nil, 0, 0)
          table.attach(gtk::Label.new('b'), 1, 2, 0, 1)
        end

        expect(attach_entries).to be_empty
      end
    end
