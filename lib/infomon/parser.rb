...
module Infomon
  ...
  module Parser
    module Pattern
      ...
      CharRaceProf = /^Name:\s+(?<name>[A-z\s']+)\s+Race:\s+(?<race>[A-z]+|[A-z]+(?: |-)[A-z]+)\s+Profession:\s+(?<profession>[-A-z]+)/.freeze # serves as StatStart
      ...
      PSM = %r{^\s+(?<name>[\w\s\-']+)\s+(?<command>[a-z]+)\s+(?<ranks>\d)/(?<max>\d)\s+}.freeze
