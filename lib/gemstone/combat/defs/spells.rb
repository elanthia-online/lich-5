        ...
        module Attacks
          ...
          SPELL_ATTACKS = [
            ...
            AttackDef.new(:divine_wrath, [
              ...
              /The ground shakes powerfully directly underneath (?<target>[^ ]+) as a beam of golden light lances away from (?:you|.+?) to transfix \k<target>\./,
              /As several faintly glowing snowflakes settle upon (?<target>[^,]+), they ignite with cold white flame\./,
              /Guided by the knowledge within you, you concentrate upon (?<target>[^,]+), and, by strength of will alone, you expose the natural weaknesses of \k<target>\./,
              ...
              /A tendril of black mist suddenly senses the proximity of (?<target>[^.]+)\. The mist expands into a large, quickly moving, highly lethal cloud that rapidly surrounds \k<target>, obliterating it from view\./,
              ...
              /a tendril of scarlet mist lashes away from you to coil around (?<target>[^ ]+) with a suddenly razor-sharp edge\./,
              ...
              /A shadowy black rose touches (?<target>[^ ]+) and wraps immediately about it, struggling to force its long, barbed thorns into the \k<target>\./,
              /A dagger of ivory light suddenly flashes away from the aura surrounding you to strike at (?<target>[^.]+)\./,
              /the intense waves of elegant sound focus upon (?<target>[^.]+)\./,
