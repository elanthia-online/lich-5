(() => {
  ...
  const renderers = {
    ...
    grid(page, component) {
      ...
      grid.style.gridTemplateColumns = Array.isArray(weights) && weights.length
        ? weights.map((weight) => (weight > 0 ? `${weight}fr` : "auto")).join(" ")
        : `repeat(${component.props.cols}, auto)`;
  ...
  function placeMenuLayer(page, menu, level, x, y) {
  ...
  function menuItem(page, item, level) {
    ...
    if (submenu) {
      ...
      const open = () => {
        const rect = button.getBoundingClientRect();
        placeMenuLayer(page, submenu, level + 1, rect.right, rect.top);
      };
      button.addEventListener("mouseenter", open);
      button.addEventListener("click", open);
      button.addEventListener("keydown", (event) => { if (event.key === "ArrowRight") { event.preventDefault(); open(); } });
    } else {
      button.addEventListener("mouseenter", () => closeMenuLayers(level + 1));
      button.addEventListener("click", () => {
        if (props.kind === "check") emit(page, item, "change", { value: props.active !== true });
        else if (props.kind === "radio") emit(page, item, "change", { value: true });
        emit(page, item, "activate");
        closeMenus();
      });
    }
  ...
  function applyFacilities(page) {
    ...
    if (facilities.presentation && Number.isFinite(facilities.presentation.opacity)) {
      page.element.style.opacity = String(facilities.presentation.opacity);
    } else if (page.element.style.opacity) {
  ...
  function receive(event) {
    ...
    if (message.type === "hello" || message.type === "pages") {
      ...
      message.pages.forEach((descriptor) => {
        if (onlyPage && descriptor.address !== onlyPage) return;
