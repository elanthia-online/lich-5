(() => {
  ...
  let socket;
  // Events sent and not yet answered, keyed by page+cid+event so a refusal
  // can be matched to the event it actually refers to. Each record carries
  // the payload and submission it was sent with and how many times it has
  // been retried, so a replay uses the original input rather than whatever
  // the controls hold by then, and cannot be retried forever.
  const pendingEvents = new Map();
  // Each event the browser sends carries its own request id and the record
  // is kept under it, so two sends from the same button are two records
  // and a refusal names exactly the one it refused. A record is dropped by
  // the refusal that answers it or by age -- never by a render, because a
  // render is not an acknowledgement of anything in flight: an unrelated
  // refresh landing between the send and its refusal used to erase the
  // record, and the refusal then found nothing to replay (review
  // 2026-09-17, R5). Accepted events are never refused, so their records
  // age out instead.
  let requestCounter = 0;
  const PENDING_TTL_MS = 30_000;
  // A record's life is bounded by more than the next render of its page
  // (review 2026-09-17 (b), F5): it used to be swept only when that page
  // rendered again, so an idle or closed page kept its records -- and the
  // password each carried as its submission -- indefinitely, and a refusal
  // arriving after the TTL was honoured before the age was checked. Now
  // every send and every message received sweep the expired ones, a
  // closed page takes its records with it, a control the script cleared
  // takes every record that carried its value, a dropped socket drops
  // them all (a reconnect resumes from the server's state, not from
  // these), and the set is capped. No timer: one would hold the page's
  // event loop, and the harness with it, for nothing.
  const PENDING_LIMIT = 256;
  function expired(record, now) {
    return now - record.sentAt > PENDING_TTL_MS;
  }
  function sweepPending(now = Date.now()) {
    pendingEvents.forEach((record, key) => { if (expired(record, now)) pendingEvents.delete(key); });
    if (pendingEvents.size > PENDING_LIMIT) {
      const oldest = Array.from(pendingEvents.keys()).sort((a, b) => a - b).slice(0, pendingEvents.size - PENDING_LIMIT);
      oldest.forEach((key) => pendingEvents.delete(key));
    }
  }
  function dropPending(matches) {
    pendingEvents.forEach((record, key) => { if (matches(record)) pendingEvents.delete(key); });
  }
  // Pages attached on the CURRENT socket. Cleared when the socket goes, so a
  // reconnect re-attaches and resumes; a catalog broadcast on a live socket
  // does not.
  let attachedPages = new Set();
  const MAX_EVENT_ATTEMPTS = 1;

  function requestKey(address, cid, event) {
    return `${address}|${cid}|${event}`;
  }
  // The row a multi-selection extends from on shift-click, per table. Kept
  // outside the page since a render rebuilds the page's controls.
  ...
  // What a script stores and later re-declares as its geometry facility:
  // the content size, which is what fitWindow takes geometry to be. It
  // used to report the outer size, so every launch added the title bar
  // and borders again and the window grew until the screen stopped it.
  ...
  function trackWindowGeometry(page, component) {
    ...
    // The baseline is the last geometry REPORTED, kept on the page across
    // renders. It used to be re-sampled here, and every render restarts
    // this tracker, so a page that re-rendered faster than the interval (a
    // countdown redrawn every half second) had its resize swallowed: the
    // new size became the baseline before the timer could report it.
    if (page.geometryPrevious === undefined) page.geometryPrevious = windowGeometry();
    page.geometryTimer = window.setInterval(() => {
      const current = windowGeometry();
      if (current === page.geometryPrevious) return;

      page.geometryPrevious = current;
  ...
  function common(element, component) {
    ...
    // A div has no `disabled` of its own, so a container the contract lets
    // carry the prop -- table, tabs, expander -- showed nothing at all.
    else if (props.disabled === true) element.dataset.disabled = "true";
  ...
  function controlValue(control) {
    ...
    if (typeof control.chipsValues === "function") return control.chipsValues();
  ...
  // The shim's own widget key (props.key) survives a node being rebuilt
  // under a new cid, which a cid does not: an Entry whose visibility flips
  // is destroyed and created again as the other input type.
  ...
  // `submission` is passed only by a replay: the values the event was first
  // sent with, so a control edited between the send and the refusal is not
  // quietly re-submitted with its new value under the old click.
  function emit(page, component, event, payload = {}, attempt = 0, submission = undefined, replayScope = undefined) {
    ...
    if (submission !== undefined) message.submission = submission;
    else if (scope) message.submission = scope.map((cid) => controlValue(page.controls.get(cid)));
    // A replay carries the submission it was first sent with, so its record
    // keeps the scope of that submission too: recomputed from a render that
    // no longer lists the control, the record held the value with an empty
    // scope and clear_sensitive could not find it (review 2026-09-17 (c)).
    const recordScope = replayScope !== undefined ? replayScope : scope;
    ...
    //
    // The record is keyed by page+cid+event and carries its own attempt
    // count and the submission it was sent with. It used to be a single slot
    // holding only the LAST event sent, and the refusal handler replayed
    // whatever was in it without checking the refusal named that event -- so
    // send A then B, refuse A, and the browser sent A, B, B: B re-executed
    // while A was lost. The attempt count travels with the record rather
    // than being released just before the replay, which is what let one
    // event be retried over and over.
    if (!UNSOLICITED.has(event)) {
      sweepPending();
      const request = ++requestCounter;
      message.request = request;
      pendingEvents.set(request, {
        request, address: page.address, cid: component.cid, event, payload, attempt,
        submission: message.submission, scope: recordScope || [], generation: page.generation, sentAt: Date.now()
      });
    }
    socket.send(JSON.stringify(message));
  }

  // The record a refusal names. A server that echoes the request id names
  // it exactly; one that does not gets the newest record for the same
  // page, cid and event, which is what the id disambiguates.
  function refusedRecord(message) {
    if (Number.isInteger(message.request)) return pendingEvents.get(message.request) || null;
    if (!message.cid || !message.event) return null;
    let newest = null;
    pendingEvents.forEach((record) => {
      if (record.address !== message.page || record.cid !== message.cid || record.event !== message.event) return;
      if (!newest || record.request > newest.request) newest = record;
    });
    return newest;
  }
  ...
  // A cell a script declared an editor for. The shim advertises these
  // through CellRendererText and CellRendererToggle, the contract carries
  // them on the column, and the client rendered every cell as text -- so a
  // column that said it was editable was not.
  function editorCell(page, component, row, column, value) {
    const cell = node("td", "editable-cell");
    const editor = column.editor;
    const send = (edited) => emit(page, component, "cell_edit", {
      row: row.key, column: column.key, value: edited,
    });
    ...
    if (editor.type === "number") {
      control.type = "number";
      control.min = editor.min;
      control.max = editor.max;
      if (Number.isFinite(editor.step)) control.step = editor.step;
    } else {
      control.type = "text";
      if (editor.max_length) control.maxLength = editor.max_length;
    }
    control.value = value == null ? "" : value;
    // change rather than input: a cell_edit per keystroke would be a render
    // per keystroke, and the script wants the committed value.
    control.addEventListener("change", () => {
      // A committed edit is no longer a draft: the script decides what the
      // cell holds now (it owns the model, R7), and its answer -- the value
      // kept, or the old one put back -- must show, not the keystrokes.
      control.dataset.webuiBase = control.value;
      send(editor.type === "number" ? Number(control.value) : control.value);
    });
    // Registered like any other text control, under a cid of its own, so
    // captureEditing carries a half-typed cell across an unrelated render
    // (review 2026-09-17, R10). The value it was rendered with travels on
    // the element, because no component in the tree carries it as `value`.
    control.dataset.webuiBase = control.value;
    page.controls.set(`${component.cid}/cell:${row.key}/${column.key}`, control);
    cell.append(control);
    return cell;
  }
  ...
  const renderers = {
    ...
    group(page, component) {
      // collapsible is declared by the contract and was never read. A group
      // that asks for it becomes the same <details>/<summary> an expander
      // uses, so the two agree rather than one silently doing nothing.
      if (component.props.collapsible === true) {
        const details = common(node("details", "webui-group collapsible"), component);
        details.open = true;
      ...
      // A selectable group is a card: the viewer can choose it, so it needs
      // the state, the role and the keyboard that any other option has.
      if (component.props.selectable === true) {
        const selected = component.props.selected === true;
        group.classList.add("selectable");
        group.setAttribute("role", "button");
        group.setAttribute("aria-pressed", String(selected));
        if (component.props.disabled !== true) {
          group.tabIndex = 0;
          const choose = () => emit(page, component, "select", { selected: !selected });
    ...
    scroll(page, component) {
      ...
      // The contract declares scroll_to as the cid to bring into view, and the
      // validator already refuses one that names no declared region -- but
      // nothing ever scrolled. Applied after the children exist, and only when
      // the request changed, so a rebuild does not yank the viewer back.
      const target = component.props.scroll_to;
      if (target && page.scrollRequests?.get(component.cid) !== target) {
        (page.scrollRequests ||= new Map()).set(component.cid, target);
        queueMicrotask(() => {
          const element = scroll.querySelector(`[data-cid="${CSS.escape(target)}"]`);
          if (element && scroll.isConnected) element.scrollIntoView({ block: "nearest" });
        });
      }
    ...
    columns(page, component) {
      ...
      // Declared by the contract and never read: a row that wants its columns
      // packed rather than spaced at the page gap.
      if (component.props.compact === true) columns.classList.add("compact");
    ...
    tabs(page, component) {
      ...
      // The contract declares `vertical` and the CSS has always implemented
      // it; only the class it answered to was derived from a cid. The cid
      // check stays as a fallback so the accounts page, which relies on the
      // old behaviour without setting the property, does not regress.
      if (component.props.vertical === true || component.cid.includes("saved-account-tabs")) {
    ...
    image(_page, component) {
      ...
      // tricked into following. Empty means the image has nothing to show
      // yet, and an <img> with an empty src attribute is a broken image, so
      // the attribute is left off and the box keeps its place.
      if (component.props.src) image.src = component.props.src;
    ...
    composite(page, component) {
      ...
      (component.props.layers || []).forEach((layer) => {
        ...
        // A shape says where its own box starts: a line's origin is the
        // lesser of its two ends, and a stroke needs room outside the box.
        const left = element.dataset.originX !== undefined ? Number(element.dataset.originX) : (layer.x ?? layer.x1 ?? 0);
        const top = element.dataset.originY !== undefined ? Number(element.dataset.originY) : (layer.y ?? layer.y1 ?? 0);
      ...
      if (component.props.surface_events) {
        ...
        // 2.19: ctrl+wheel is a zoom request, with the pixel under the
        // pointer so the script can hold it still while it rescales. A plain
        // wheel is left to the scroller. Not passive: the browser's own
        // ctrl+wheel is page zoom, which must not fire as well.
        surface.addEventListener("wheel", (event) => {
          if (!event.ctrlKey || event.deltaY === 0) return;
          event.preventDefault();
          const payload = surfacePayload(event, surface, "primary");
          delete payload.button;
          payload.direction = event.deltaY < 0 ? "in" : "out";
          emit(page, component, "surface_zoom", payload);
        }, { passive: false });
      }
      return surface;
    },
    text(_page, component) {
      ...
      // A type scale without wrapping the content in a markup span. The
      // vocabulary is Pango's, which CSS accepts verbatim as a font-size
      // keyword, so the two spellings render identically.
      if (component.props.size) text.style.fontSize = component.props.size;
    ...
    // markdown, log and slider are declared by the contract and were never
    // rendered: an author following the schema got a "Renderer not
    // implemented" box. The shim never emits them, which is why nothing
    // noticed -- but the contract is a promise to anyone reading it.
    markdown(_page, component) {
      // rich_text is Pango-flavoured markup, the same vocabulary `text`
      // carries, parsed server-side and rebuilt here as elements.
      const wrapper = common(node("div", "webui-markdown"), component);
      renderMarkup(component.props.markup, component.props.content, wrapper);
      return wrapper;
    },
    log(_page, component) {
      const wrapper = common(node("div", "webui-log"), component);
      // max_lines is retention, and the validator already refuses more lines
      // than it allows -- honouring it here keeps a long-running log from
      // growing without bound in the DOM.
      const limit = component.props.max_lines;
      const lines = component.props.lines || [];
      const shown = Number.isFinite(limit) && lines.length > limit ? lines.slice(-limit) : lines;
      shown.forEach((line) => wrapper.append(node("div", "log-line", line)));
      // follow defaults true: a log is read at the bottom.
      if (component.props.follow !== false) {
        queueMicrotask(() => { if (wrapper.isConnected) wrapper.scrollTop = wrapper.scrollHeight; });
      }
      return wrapper;
    },
    slider(page, component) {
      ...
      control.min = component.props.min;
      control.max = component.props.max;
      if (Number.isFinite(component.props.step)) control.step = component.props.step;
      control.value = component.props.value;
      control.addEventListener("change", () => emit(page, component, "change", { value: Number(control.value) }));
      return field(page, component, control);
    },
    ...
    radio(page, component) {
      ...
      component.props.options.forEach((option) => {
        // 2.20: a heading before the first option of each group.
        if (option.group && option.group !== seenGroup) {
          seenGroup = option.group;
          options.append(node("div", "radio-group", option.group));
        }
    ...
    password_input(page, component) {
      // The control is never seeded from the tree, and must not be: the
      // tree never carries a password. What the viewer has typed reaches
      // this control only through restoreEditing's draft.
      const control = input(page, component, "password");
      const wrapper = field(page, component, control);
      // 2.18 (D17): `change` says the value changed and nothing about what
      // it is -- the payload is empty by contract, so a strength meter can
      // follow the typing without the password leaving the browser. One
      // per animation frame: a burst of keystrokes is one fact.
      let changeFrame = null;
      control.addEventListener("input", () => {
        if (changeFrame !== null) return;
        changeFrame = requestAnimationFrame(() => {
          changeFrame = null;
          emit(page, component, "change");
        });
      });
      control.addEventListener("keydown", (event) => {
        if (event.key === "Enter") emit(page, component, "submit");
    ...
    select(page, component) {
      const control = node("select");
      // The contract carries `disabled` on select and the server sends it;
      // nothing here read it, so a script that greyed out a dropdown got a
      // fully live one.
      control.disabled = component.props.disabled === true;
      // 2.20: `size` makes it a list box; grouped options sit under their
      // heading, in the order the group is first seen.
      if (component.props.size > 1) control.size = component.props.size;
      const groups = new Map();
      component.props.options.forEach((option) => {
        const choice = node("option", null, option.label);
        choice.value = option.value;
        choice.selected = option.value === component.props.value;
        if (!option.group) return control.append(choice);
        ...
        groups.get(option.group).append(choice);
      });
      control.addEventListener("change", () => emit(page, component, "change", { value: control.value }));
      return field(page, component, control);
    },
    // 2.20: several values from a list, as removable chips with a filter
    // box. The registered control is the wrapper: its `value` is the chips
    // as JSON (what editing preservation compares) and `chipsValues()` the
    // array a submission carries. The option list is built from the
    // filter's text and lives only while there is text, unless the field
    // is not searchable, in which case focus opens the whole list.
    chips(page, component) {
      const props = component.props;
      const control = node("div", "chips-control");
      const list = node("div", "chips-list");
      ...
      filter.disabled = props.disabled === true;
      if (props.placeholder) filter.placeholder = props.placeholder;
      const options = node("div", "chips-options");
      options.setAttribute("role", "listbox");
      options.hidden = true;
      let values = [...(props.value || [])];
      const labelOf = (value) => (props.options || []).find((option) => option.value === value)?.label ?? value;
      const full = () => props.max > 0 && values.length >= props.max;
      const searchable = props.searchable !== false;
      control.chipsValues = () => values.slice();
      Object.defineProperty(control, "value", {
        get: () => JSON.stringify(values),
        set: (next) => { try { values = JSON.parse(next); } catch { /* not ours */ } drawChips(); },
      });
      control.dataset.webuiBase = JSON.stringify(values);
      ...
      function drawChips() {
        ...
        values.forEach((value) => {
          ...
          chip.append(node("span", "chip-label", labelOf(value)));
          const remove = node("button", "chip-remove", "\u00D7");
          remove.type = "button";
          remove.setAttribute("aria-label", `Remove ${labelOf(value)}`);
          remove.disabled = props.disabled === true;
          remove.addEventListener("click", (event) => { event.preventDefault(); commit(values.filter((v) => v !== value)); filter.focus(); });
          chip.append(remove);
          list.append(chip);
        });
        filter.disabled = props.disabled === true || full();
      }
      function commit(next) {
        values = next;
        drawChips();
        emit(page, component, "change", { values: values.slice() });
      }
      function add(value) {
        if (!value || values.includes(value) || full()) return;
        commit([...values, value]);
        filter.value = "";
        closeOptions();
      }
      function candidates() {
        ...
        return (props.options || [])
          ...
          .filter((option) => !text || option.label.toLowerCase().includes(text) || option.value.toLowerCase().includes(text))
          .slice(0, 50);
      }
      ...
      function openOptions() {
        ...
        if ((searchable && !text) || full()) return closeOptions();
        const found = candidates();
        if (found.length === 0 && !(props.allow_custom && text)) return closeOptions();
        options.replaceChildren();
        let group = null;
        found.forEach((option) => {
          if (option.group && option.group !== group) {
            group = option.group;
            options.append(node("div", "chips-group", group));
          }
          const item = node("div", "chips-option", option.label);
          item.setAttribute("role", "option");
          item.dataset.value = option.value;
          item.addEventListener("mousedown", (event) => { event.preventDefault(); add(option.value); });
          options.append(item);
        });
        if (props.allow_custom && text && !found.some((option) => option.value === text)) {
          const item = node("div", "chips-option custom", `Add "${text}"`);
          item.setAttribute("role", "option");
          item.dataset.value = text;
          item.addEventListener("mousedown", (event) => { event.preventDefault(); add(text); });
          options.append(item);
        }
        options.hidden = false;
        filter.setAttribute("aria-expanded", "true");
      }
      filter.addEventListener("input", openOptions);
      filter.addEventListener("focus", () => { if (!searchable) openOptions(); });
      filter.addEventListener("blur", closeOptions);
      filter.addEventListener("keydown", (event) => {
        if (event.key === "Enter") {
          event.preventDefault();
          const first = options.querySelector(".chips-option");
          if (first) add(first.dataset.value);
        } else if (event.key === "Escape") {
          closeOptions();
        } else if (event.key === "Backspace" && filter.value === "" && values.length > 0) {
          commit(values.slice(0, -1));
        }
      });
      drawChips();
      control.append(list, filter, options);
      return field(page, component, control);
    },
    // A selection list with per-item state. Rendered as a real listbox so
    // arrow keys, Home/End and the active-descendant relationship come from
    // the platform rather than from each script reinventing them -- which is
    // the reason this is a primitive and not a stack of buttons.
    nav(page, component) {
      const wrapper = common(node("div", "webui-nav"), component);
      const list = node("div", "nav-list");
      list.setAttribute("role", "listbox");
      list.tabIndex = component.props.disabled === true ? -1 : 0;
      ...
      items.forEach((item) => {
        if (item.section && item.section !== seenSection) {
          seenSection = item.section;
          const heading = node("div", "nav-section", item.section);
          heading.setAttribute("role", "presentation");
          list.append(heading);
        }
        const option = node("div", "nav-item");
        option.setAttribute("role", "option");
        option.dataset.itemId = item.id;
        option.dataset.status = item.status || "none";
        const active = item.id === selected;
        option.setAttribute("aria-selected", String(active));
        if (item.disabled === true) option.setAttribute("aria-disabled", "true");
        if (active) option.id = `${component.cid}-active`;

        const body = node("div", "nav-item-body");
        body.append(node("div", "nav-item-label", item.label));
        if (item.detail) body.append(node("div", "nav-item-detail", item.detail));
        option.append(node("span", "nav-item-marker"));
        option.append(body);
        if (item.badge) option.append(node("span", "nav-item-badge", item.badge));

        if (item.disabled !== true && component.props.disabled !== true) {
          option.addEventListener("click", () => emit(page, component, "select", { id: item.id }));
          options.push(option);
        }
        list.append(option);
      });

      if (selected) list.setAttribute("aria-activedescendant", `${component.cid}-active`);
      list.addEventListener("keydown", (event) => {
        if (component.props.disabled === true || !selectable.length) return;
        const at = selectable.findIndex((item) => item.id === selected);
        let next = null;
        if (event.key === "ArrowDown") next = selectable[Math.min(at + 1, selectable.length - 1)];
        else if (event.key === "ArrowUp") next = selectable[Math.max(at - 1, 0)];
        else if (event.key === "Home") next = selectable[0];
        else if (event.key === "End") next = selectable[selectable.length - 1];
        else return;
        event.preventDefault();
        if (next && next.id !== selected) emit(page, component, "select", { id: next.id });
      });
      wrapper.append(list);
      return wrapper;
    },
    table(page, component) {
      ...
      // max_height is declared on table and was never applied, so a long
      // catalog grew to its full height and pushed whatever followed it off
      // the page instead of scrolling inside its own box.
      if (Number.isFinite(component.props.max_height)) {
        wrapper.style.maxHeight = `${component.props.max_height}px`;
      }
      ...
      if (component.props.headers !== false) {
        ...
        // The contract carries `sortable` on the table and on each column, and
        // the validator already refuses a sort on a column that is not both --
        // so a header that cannot be clicked made the server enforce rules
        // about a feature the viewer had no way to use.
        const sortable = component.props.sortable === true;
        const sort = component.props.sort;
        component.props.columns.forEach((column) => {
          const cell = node("th", null, column.label);
          if (sortable && column.sortable === true) {
            ...
            const request = () => emit(page, component, "sort_change", { column: column.key, direction: next });
      ...
      // 2.10 declared parent/expanded/row_toggle and the shim admitted
      // TreeStore, but every row was laid out as a sibling: a collapsed
      // parent still showed its children and nothing could expand or
      // collapse anything (review 2026-09-17, R14). Rows are walked as the
      // tree they describe; a collapsed parent's descendants are not drawn;
      // a parent gets a toggle in its first cell.
      const rows = component.props.rows;
      const keys = new Set(rows.map((candidate) => candidate.key));
      const childrenOf = new Map();
      rows.forEach((candidate) => {
        if (candidate.parent && keys.has(candidate.parent)) {
          if (!childrenOf.has(candidate.parent)) childrenOf.set(candidate.parent, []);
          childrenOf.get(candidate.parent).push(candidate);
        }
      });
      const toggleRow = (row, expanded) => {
        // The viewer's own state, whether or not the script listens: it is
        // told when it does (row_toggle is viewer state and re-renders), and
        // the table folds locally either way.
        row.expanded = expanded;
        wrapper.replaceWith(renderers.table(page, component));
        emit(page, component, "row_toggle", { row: row.key, expanded });
      };
      const renderRow = (row, depth) => {
        ...
        tr.addEventListener("click", (event) => {
          if (component.props.disabled === true) return;
          const mode = component.props.selection;
          if (mode === "none") return;
          let rows = [row.key];
          if (mode === "multi") {
            // GTK's multiple selection: a plain click selects the row alone,
            // ctrl toggles it within the selection, shift extends from the
            // last row clicked. The selection is read from the rows on
            // screen rather than from props, because the render confirming
            // one click has not necessarily landed before the next.
            const anchorKey = requestKey(page.address, component.cid, "anchor");
            const keys = component.props.rows.map((candidate) => candidate.key);
            const current = Array.from(body.querySelectorAll('tr[aria-selected="true"]'))
              .map((selected) => selected.dataset.rowKey);
            if (event.shiftKey && keys.includes(selectionAnchors.get(anchorKey))) {
              const [from, to] = [keys.indexOf(selectionAnchors.get(anchorKey)), keys.indexOf(row.key)].sort((a, b) => a - b);
              rows = keys.slice(from, to + 1);
            } else if (event.ctrlKey || event.metaKey) {
              rows = current.includes(row.key) ? current.filter((key) => key !== row.key) : current.concat(row.key);
            }
            if (!event.shiftKey) selectionAnchors.set(anchorKey, row.key);
            body.querySelectorAll("tr").forEach((candidate) => {
              candidate.setAttribute("aria-selected", String(rows.includes(candidate.dataset.rowKey)));
            });
          }
          emit(page, component, "selection_change", { rows });
        });
        // The shim binds row-activated whenever a script connects to it --
        // ewaggle, repository and jinx all do -- and the client emitted
        // nothing, so the gesture was dead. GTK activates on double-click and
        // on Enter, so both do here.
        const activate = () => {
          if (component.props.disabled !== true) emit(page, component, "row_activate", { row: row.key });
        };
        tr.addEventListener("dblclick", activate);
        tr.addEventListener("keydown", (event) => {
          if (event.key === "Enter") { event.preventDefault(); activate(); }
        });
        tr.tabIndex = component.props.disabled === true ? -1 : 0;
        component.props.columns.forEach((column, index) => {
          const value = row.cells[column.key];
          let cell;
          if (column.editor && component.props.disabled !== true) {
            cell = editorCell(page, component, row, column, value);
          } else {
            cell = node("td", null, value);
          }
          if (index === 0 && (depth > 0 || childrenOf.has(row.key))) {
            cell.classList.add("tree-cell");
            cell.style.paddingLeft = `${9 + depth * 18}px`;
            if (childrenOf.has(row.key)) {
              const expanded = row.expanded === true;
              const toggle = node("button", "tree-toggle", expanded ? "\u25BE" : "\u25B8");
              toggle.type = "button";
        ...
        body.append(tr);
        if (row.expanded === true) (childrenOf.get(row.key) || []).forEach((child) => renderRow(child, depth + 1));
      };
      rows.forEach((row) => {
        if (!row.parent || !keys.has(row.parent)) renderRow(row, 0);
      });
      table.append(body);
  ...
  function compositeLayer(page, component, layer) {
    if (layer.kind === "line" || layer.kind === "rect" || layer.kind === "ellipse") return shapeLayer(layer);
    if (layer.kind === "image") {
      const image = node("img");
      if (layer.src) image.src = layer.src;
      image.alt = "";
  ...
  // 2.17: a drawn shape, as an SVG sized to the shape's own box plus room
  // for its stroke, so the composite can place it like any other layer.
  // Built with createElementNS as the rest of the tree is built with
  // createElement: a layer is validated data, never markup.
  function svgNode(tag) {
    return document.createElementNS("http://www.w3.org/2000/svg", tag);
  }

  function shapeLayer(layer) {
    const strokeWidth = layer.stroke ? (Number.isFinite(layer.stroke_width) ? layer.stroke_width : 1) : 0;
    ...
    if (layer.kind === "line") {
      ...
      shape = svgNode("line");
      shape.setAttribute("x1", String(layer.x1 - x + pad));
      shape.setAttribute("y1", String(layer.y1 - y + pad));
      shape.setAttribute("x2", String(layer.x2 - x + pad));
      shape.setAttribute("y2", String(layer.y2 - y + pad));
    } else {
      x = layer.x;
      y = layer.y;
      w = layer.w;
      ...
      if (layer.kind === "rect") {
        shape = svgNode("rect");
        shape.setAttribute("x", String(pad));
        shape.setAttribute("y", String(pad));
        shape.setAttribute("width", String(w));
        shape.setAttribute("height", String(h));
      } else {
        shape = svgNode("ellipse");
        shape.setAttribute("cx", String(w / 2 + pad));
        shape.setAttribute("cy", String(h / 2 + pad));
        shape.setAttribute("rx", String(w / 2));
        shape.setAttribute("ry", String(h / 2));
      }
    }
    shape.setAttribute("fill", layer.fill ? cssColor(layer.fill) : "none");
    shape.setAttribute("stroke", layer.stroke ? cssColor(layer.stroke) : "none");
    if (layer.stroke) shape.setAttribute("stroke-width", String(strokeWidth));
    const svg = svgNode("svg");
    svg.classList.add("composite-shape");
    svg.setAttribute("width", String(w + pad * 2));
    svg.setAttribute("height", String(h + pad * 2));
    svg.setAttribute("aria-hidden", "true");
    if (Number.isFinite(layer.opacity) && layer.opacity !== 1) svg.style.opacity = String(layer.opacity);
    svg.dataset.originX = String(x - pad);
    svg.dataset.originY = String(y - pad);
    svg.append(shape);
    return svg;
  }
  ...
  function surfacePayload(event, surface, button) {
    ...
    if (event.shiftKey) modifiers.push("shift");
    if (event.altKey) modifiers.push("alt");
    // The pixel under the cursor in the surface's own coordinates. Left
    // unrounded until the scroll offset has come off: rounding each side
    // separately and then subtracting could land a pixel away from
    // rounding the difference once. The offset is an integer once rounded,
    // so round(raw - offset) === round(raw) - offset, and the script's
    // `x + scroll_x` gives back the absolute pixel exactly.
    const rawX = (event.clientX - rect.left) / s;
    const rawY = (event.clientY - rect.top) / s;
    ...
    const payload = { x: Math.round(rawX - sx), y: Math.round(rawY - sy), button, modifiers };
    if (scroller) {
      payload.scroll_x = sx;
      payload.scroll_y = sy;
    }
  ...
  function cssColor(tint) {
    ...
    // A tint is a tone record or an RGBA record (contract TINT). A tone
    // used to fall through to the RGBA branch and paint black: the harness
    // caught a `{ tone: "danger" }` stroke drawn as rgb(0 0 0).
    if (typeof tint.tone === "string") return `var(--tone-${tint.tone})`;
  ...
  // Sizes the window to the page, once, on its first render. A window the
  // browser opened for one page comes up at whatever size Chrome used last
  // (--window-size is ignored once Chrome is running), so the first thing a
  // page did was appear the size of the screen. An explicit `geometry`
  // facility is honoured as the window size; otherwise the window takes
  // the content's natural size, which is what a GTK window did.
  const FIT_MIN = { width: 320, height: 200 };

  function fitWindow(page) {
    if (page.fitted || pages.size !== 1 || typeof window.resizeTo !== "function") return;
    page.fitted = true;
    // GTK's rule: a default size is honoured unless the content's natural
    // size is larger, in which case the content wins -- a window is never
    // opened smaller than what it has to show. So the natural size is
    // measured even when geometry was declared, and each axis takes the
    // larger. An axis the geometry left at 0 is the content's alone.
    const geometry = page.facilities?.geometry || {};
    const measured = naturalPageSize();
    page.fittedNatural = measured;
    const natural = measured || { width: 0, height: 0 };
    const inner = {
      width: Math.max(geometry.width > 0 ? geometry.width : 0, natural.width),
      height: Math.max(geometry.height > 0 ? geometry.height : 0, natural.height),
    };
    ...
    const limit = { width: window.screen?.availWidth || Infinity, height: window.screen?.availHeight || Infinity };
    const width = Math.min(limit.width, Math.max(FIT_MIN.width, Math.ceil(inner.width + chrome.width)));
    const height = Math.min(limit.height, Math.max(FIT_MIN.height, Math.ceil(inner.height + chrome.height)));
    window.resizeTo(width, height);
    // The declared position, the same way: Chrome ignores --window-position
    // once it is running, so a window that was to come back where the
    // viewer left it came back wherever Chrome put it.
    const x = geometry.x;
    const y = geometry.y;
    if (Number.isFinite(x) && Number.isFinite(y) && typeof window.moveTo === "function") window.moveTo(x, y);
  }

  // The content's own size, measured with the page laid out as an inline
  // block so it neither stretches to the viewport nor scrolls inside it.
  // Null when nothing can be measured (no layout engine).
  // Content that grows or shrinks after the fit -- the launcher's GUI
  // Settings folding open under the tabs -- moves the window's bottom edge
  // by the same amount, so what opened is in view and what closed gives
  // the space back. The width is left alone: that is the viewer's.
  function refitWindow(page) {
    if (!page.fitted || pages.size !== 1 || typeof window.resizeTo !== "function") return;
    const natural = naturalPageSize();
    if (!natural) return;
    const previous = page.fittedNatural;
    page.fittedNatural = natural;
    if (!previous) return;
    const delta = Math.ceil(natural.height - previous.height);
    if (Math.abs(delta) < 2) return;
    const limit = window.screen?.availHeight || Infinity;
    const height = Math.min(limit, Math.max(FIT_MIN.height, window.outerHeight + delta));
    window.resizeTo(window.outerWidth, height);
  }

  function naturalPageSize() {
    document.documentElement.classList.add("webui-measuring");
    try {
      const rect = pagesNode.getBoundingClientRect();
      if (!(rect.width > 0 && rect.height > 0)) return null;
      const body = window.getComputedStyle(document.body);
      const pad = (side) => parseFloat(body[side]) || 0;
      return {
        width: rect.width + pad("paddingLeft") + pad("paddingRight"),
        height: rect.height + pad("paddingTop") + pad("paddingBottom"),
      };
    } finally {
      document.documentElement.classList.remove("webui-measuring");
    }
  }

  function applyFacilities(page) {
    const facilities = page.facilities || {};
    // Geometry sizes the window (fitWindow); it styles the page only while
    // the page shares a window with others. Once the window has been fitted
    // to this page the page fills it, and restyling it from geometry on
    // every render pinned the page to the stored size: a viewer widening
    // the window got a page that stayed narrow, and a script re-declaring
    // the reported outer size got a page wider than the viewport.
    if (facilities.geometry && !(pages.size === 1 && page.fitted)) {
    ...
    // A notice a re-render repeats is not news. The server drops its own
    // once rendered, but a render already in flight when it did still
    // carries it, and each one used to add another toast.
    const notice = facilities.notify ? `${facilities.notify.level}\n${facilities.notify.text}` : null;
    if (notice && notice !== page.lastNotice) notify(facilities.notify.text, facilities.notify.level);
    page.lastNotice = notice;
  ...
  // been sent, so re-applying a stale one would fight the server. A number
  // field is text-like: it commits on change, so digits typed and not yet
  // committed are a draft like any other, and leaving it out let a render
  // put the old server value back under a half-typed one.
  function captureEditing(page) {
    ...
    for (const [cid, control] of page.controls || []) {
      ...
      const editable = control.tagName === "TEXTAREA"
        || (control.tagName === "INPUT" && !["checkbox", "radio", "range"].includes(control.type));
      // Text the browser cannot parse as a number ("4.", "-") is reported as
      // an empty value with badInput set. It cannot be carried across a DOM
      // swap -- the new control could only be given "", not the text -- so
      // it is not recorded as a draft, and the server value stands.
      if (control.type === "number" && control.validity?.badInput) continue;
      // The component's own key travels with the draft: a script flipping an
      // Entry's visibility rebuilds its node under a new cid, and without the
      // key the draft would look up a cid the new tree no longer has.
      const component = findComponent(page.tree, cid);
      const key = component?.props?.key ?? null;
      if (control === active) {
        state.focusCid = cid;
        state.focusKey = key;
        if (editable) {
      ...
      // below.
      const rendered = control.dataset.webuiBase !== undefined ? control.dataset.webuiBase : component?.props?.value;
      if (editable && rendered !== undefined && control.value !== String(rendered)) {
        state.edits.set(cid, { typed: control.value, base: String(rendered), key });
      } else if (editable && rendered === undefined && control.type === "password" && control.value) {
        // A password never carries `value` in the tree -- deliberately, it is
        // write-only -- so the guard above skipped it and any unrelated
        // render (a geometry update, a tab switch) wiped a half-typed
        // password. There is no server base to compare against, so the draft
        // is simply the viewer's own text: keep it, with base null so the
        // restore side knows the server never had an opinion.
        state.edits.set(cid, { typed: control.value, base: null, key });
      }
    }
    return state;
  }

  // The control a draft belongs to in the tree just rendered: the same cid
  // when the node survived, otherwise the node carrying the same shim key.
  // That second path is how a half-typed password follows an Entry whose
  // visibility a script turned on ("show password"): the password_input is
  // destroyed and a text_input created under a new cid, and the masked
  // typing -- which never had a channel to the server -- lands in the new
  // control. The reverse (text_input becoming password_input) carries the
  // typed text into the new masked control the same way, in the browser
  // only: password_input has no value in the tree and never gets one.
  function editingControl(page, cid, key) {
    const direct = page.controls?.get(cid);
    if (direct) return direct;
    const replacement = findComponentByKey(page.tree, key);
    return replacement ? page.controls?.get(replacement.cid) ?? null : null;
  }

  function restoreEditing(page, state) {
    if (!state) return;
    for (const [cid, edit] of state.edits) {
      const control = editingControl(page, cid, edit.key);
      if (!control) continue;
      ...
      const rendered = control.dataset.webuiBase !== undefined
        ? control.dataset.webuiBase
        : findComponent(page.tree, cid)?.props?.value;
      // base null is a password draft: the server has no value for it, so
      // there is nothing that could have changed under the viewer. An
      // explicit clear_sensitive still empties it, and a removed component
      // has no control to restore into.
      if (edit.base !== null && rendered !== undefined && String(rendered) !== edit.base) continue;
      control.value = edit.typed;
    }
    if (!state.focusCid) return;
    const control = editingControl(page, state.focusCid, state.focusKey);
  ...
  function acceptRender(message) {
    ...
    // A record a stale refusal marked for replay is replayed into this
    // render: it is the state the event was waiting for. Every other record
    // stays until its own refusal or its age answers it; a render says
    // nothing about which in-flight events the server has seen.
    const replays = [];
    sweepPending();
    pendingEvents.forEach((record, key) => {
      if (record.address !== message.page || !record.replay) return;
    ...
    // Replayed into the tree just rendered, with the payload and submission
    // of the original send, one attempt further along so a second refusal
    // stops it rather than starting it over.
    replays.forEach((retry) => {
      const component = findComponent(page.tree, retry.cid);
      if (!component) return;
      window.setTimeout(() => {
        emit(page, component, retry.event, retry.payload, retry.attempt + 1, retry.submission, retry.scope);
      }, 0);
    });
  }
  ...
  function receive(event) {
    ...
    if (message.type === "hello" || message.type === "pages") {
      ...
      message.pages.forEach((descriptor) => {
        ...
        // Both `hello` and `pages` arrive for things like a sibling modal
        // opening, and re-attaching a page this socket already holds is
        // refused by the server ("viewer is already attached") -- so opening
        // a modal produced refusal notifications for messages the viewer
        // never caused, and each one consumed a pending retry slot. Only new
        // pages are attached; a genuine reconnect starts with an empty set
        // and so still resumes everything.
        if (attachedPages.has(descriptor.address)) return;
        attachedPages.add(descriptor.address);
      ...
      // The value is gone from the field; a record that would replay it is
      // not a recovery, it is a second submission of what the script
      // already consumed.
      dropPending((record) => record.scope.some((cid) => message.cids.includes(cid)));
    } else if (message.type === "page_closed") {
      window.clearInterval(pages.get(message.page)?.geometryTimer);
      dropPending((record) => record.address === message.page);
      pages.get(message.page)?.element?.remove();
      pages.delete(message.page);
      attachedPages.delete(message.page);
      // A window showing nothing is not a window anyone wants: a script
      // window whose page closed used to stay open, empty, because nothing
      // on the server monitors the browser any more. The last page to close
      // takes the window with it; Chrome allows an app window with a
      // single-entry history to close itself.
      if (pages.size === 0) window.close();
    } else if (message.type === "refusal") {
      // A stale generation is not a failure the person needs to see: the
      // server follows this refusal with the render that supersedes the
      // event, and the click is replayed against that one -- once, so a
      // genuinely rejected event cannot loop.
      // Correlated: replay the event this refusal names, never merely the
      // last one sent. Without the cid/event check a refusal for A replayed
      // B, which may already have run.
      // Age first: a refusal that arrives after the record's TTL names an
      // event the viewer gave up on, and marking it for replay used to
      // bypass the expiry entirely.
      sweepPending();
      const retry = refusedRecord(message);
      if (message.reason === "stale_generation" && retry && retry.attempt < MAX_EVENT_ATTEMPTS) {
        // Not replayed here: the server follows a stale refusal with the
        // render that superseded the event, and that render is the state
        // to replay into; acceptRender does it when it lands. A refresh
        // that happened to land first does not matter any more, because
        // the record survived it.
        retry.replay = true;
        return;
      }
      if (retry) pendingEvents.delete(retry.request);
  ...
  function connect() {
    ...
    // Every handler below closes over THIS socket rather than reading the
    // module-level `socket`, which by the time a late event fires may already
    // be its replacement. The error handler called socket.close() on whatever
    // was current, so an error from a dead socket closed the live one and the
    // close handler then dialled again: a reconnect loop that fed itself, and
    // the reason a dropped connection sometimes never came back.
    const live = new WebSocket(`${scheme}//${window.location.host}/ws`);
    socket = live;
    live.addEventListener("open", () => { reconnectDelay = 250; });
    live.addEventListener("message", receive);
    live.addEventListener("close", () => {
      // A superseded socket closing is not the live connection dropping.
