(() => {
  ...
  function emit(page, component, event, payload = {}, attempt = 0) {
  ...
  function captureEditing(page) {
    ...
    for (const [cid, control] of page.controls || []) {
      ...
      if (editable && rendered !== undefined && control.value !== String(rendered)) {
        ...
        state.edits.set(cid, { typed: control.value, base: null });
      }
    }
    return state;
  }
  ...
  function acceptRender(message) {
    ...
    pendingEvents.forEach((record, key) => {
      if (record.address === message.page) pendingEvents.delete(key);
    });
  ...
  function receive(event) {
    ...
    if (message.type === "hello" || message.type === "pages") {
      ...
      if (message.reason === "stale_generation" && retry) {
        const page = pages.get(retry.address);
        const component = page && findComponent(page.tree, retry.cid);
        if (page && component && retry.attempt < MAX_EVENT_ATTEMPTS) {
          window.setTimeout(() => {
            // The attempt count rides with the replay, so the next refusal
            // for it stops rather than starting over.
            emit(page, component, retry.event, retry.payload, retry.attempt + 1);
          }, 0);
          return;
        }
      }
