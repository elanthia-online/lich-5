(() => {
  ...
  function emit(page, component, event, payload = {}, attempt = 0, submission = undefined) {
    ...
    if (!UNSOLICITED.has(event)) {
      pendingEvents.set(requestKey(page.address, component.cid, event), {
        address: page.address, cid: component.cid, event, payload, attempt,
      ...
      component.props.rows.forEach((row) => {
        ...
        component.props.columns.forEach((column) => {
          ...
          if (column.editor && component.props.disabled !== true) {
            tr.append(editorCell(page, component, row, column, value));
          } else {
  ...
  function captureEditing(page) {
    ...
    for (const [cid, control] of page.controls || []) {
      ...
      // below.
      const rendered = component?.props?.value;
  ...
  function acceptRender(message) {
    ...
    // A render for this page answers everything outstanding against it: the
    // server has moved on, and a record nobody will refuse now would only
    // grow the map. The exception is a record a stale refusal has marked
    // for replay: this render is the state it was waiting for.
    const replays = [];
    pendingEvents.forEach((record, key) => {
      if (record.address !== message.page) return;
      pendingEvents.delete(key);
      if (record.replay) replays.push(record);
    });
  ...
  function receive(event) {
    ...
    if (message.type === "hello" || message.type === "pages") {
      ...
      const refusedKey = message.cid && message.event
        ? requestKey(message.page, message.cid, message.event)
        : null;
      const retry = refusedKey ? pendingEvents.get(refusedKey) : null;
      if (message.reason === "stale_generation" && retry && retry.attempt < MAX_EVENT_ATTEMPTS) {
        // Not replayed here: the tree this page holds now is the one the
        // event was stale against. The render that follows this refusal is
        // the state to replay into, and acceptRender does it when it lands.
        retry.replay = true;
        return;
      }
      if (refusedKey) pendingEvents.delete(refusedKey);
