(() => {
  ...
  function emit(page, component, event, payload = {}, attempt = 0, submission = undefined) {
    ...
    if (!UNSOLICITED.has(event)) {
      ...
      pendingEvents.set(request, {
        ...
        submission: message.submission, scope: scope || [], generation: page.generation, sentAt: Date.now()
      });
    }
  ...
  function acceptRender(message) {
    ...
    replays.forEach((retry) => {
      ...
      if (!component) return;
      window.setTimeout(() => {
        emit(page, component, retry.event, retry.payload, retry.attempt + 1, retry.submission);
      }, 0);
    });
  }
